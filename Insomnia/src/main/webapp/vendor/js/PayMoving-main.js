...
// ======= 주문자 정보(정보와 같음 클릭 시) -> 예매정보로 정보 이동 ============
	$(document).on("click","#same",function(){
		if($(this).get(0).checked){
			...
			$("#sample4_postcode").val($("#postcode").val());
			$("#sample4_roadAddress").val($("#roadAddress").val());
			$("#sample4_jibunAddress").val($("#jibunAddress").val());
			$("#sample4_detailAddress").val($("#detailAddress").val());
			$("#sample4_extraAddress").val($("#extraAddress").val());
...
re	*/
	...
	//ajax 처리 2019 05 08 임한결 추가 - ajax로 json을 가져와 파싱해서 추가.
	//foreach로 돌리자
	//showRewardList();
...
$(document).on("click",".btn_order_ok_main",function(){
	//1. 주문리스트 제품]
	...
	// =============================== 휴대폰 결제 ===============================
		...
		/////////// 파라미터 얻어오기
		//필요한 정보들 
		/* id, r_no, sp_reward_qty, sp_support, sp_date, 
		 * sp_recipient,
		 * sp_zip_code, sp_shipping_address, sp_depositor, sp_bank_name
		 * sp_account_serial, sp_phone 
		 */ 
		...
		//r_no, sp_reward_qty
		//주문 개수 얻어옴
		var reward_count = $("#reward_count").val()-1;
		json.reward_count = new String(reward_count);
		
		for(var i = 0; i < reward_count; i++){
			json["r_no_"+i] = $("#r_no_"+i).val();
			json["sp_reward_qty_"+i] = $("#sp_reward_qty_"+i).attr("value");
		}//for
		
		
		//2. 사용자 정보 + 후원금 정보 + 주문 메세지
		json.support = $("#support").val();
		json.sp_recipient = $("#receiver").val();
		json.sp_zip_code = $("#sample4_postcode").val();
		json.sp_shipping_address = $("#sample4_roadAddress").val()+"_"+$("#sample4_jibunAddress").val()+"_"+$("#sample4_detailAddress").val()+"_"+$("#sample4_extraAddress").val();
		if($("#sp_depositor").val() == ""){
			json.sp_depositor = $("#receiver").val();
		}///입금자명이 주문자와 같을 경우 같을 경우
		else{json.sp_depositor = $("#sp_depositor").val();}
		json.sp_bank_name = $("#sp_bank_name").val();
		json.sp_account_serial = $("#sp_account_serial").val();
		json.sp_phone = $("#emergency21").val()+$("#emergency22").val()+$("#emergency23").val();
		json.message = $("#message").val();
		json.s_no = $("#s_no").val();
		json.total_payment = $("#total_payment").attr("value");
		json.b_name = $("#b_name").val();
		json.b_no = $("#b_no").val();
		
		var resultString = JSON.stringify(json);
		console.log("result : " +resultString);
		
		
		$.ajax({
			url: "/insomnia/Pay/MainPayInsert.ins",
			data : json,
			dataType : 'json',
...
var successPay = function(data){
	...
	IMP.request_pay({
		...
		merchant_uid : 'merchant_' + data["uid"],
		/*
		 merchant_uid에 경우
		...
		name : $("#b_name").val(),
		//결제할 금액
		amount : $("#total_payment").attr("value"),
		//구매정보
		//buyer_email : full_email,
		//buyer_name : $("#receiver").val(),
		buyer_tel : $("#emergency21").val()+"-"+$("#emergency22").val()+"-"+$("#emergency23").val(),
...
//			msg += '에러내용 : ' + rsp.error_msg;
		...
		if(isComplete){
			...
			$.each(data, function(key, value){
				console.log("index : " + key + ", element : " + value);
				paramString += key+"="+value+"&";
			});
			console.log("paramString : " + paramString);
				
			location.href="/insomnia/Pay/MainPayResult.ins"+paramString;
		}
	});
}//success function
...
//임한결 추가 2019 05 09 총합 자동 계산함수
function sum(){
	var sum = 0;
	//[name^=value] 주어진 문자열로 시작하는 속성을 가진 노드
	$("span[id^=price_concert]").each(function(){
		sum += parseInt(String($(this).text().replace(",", "")));
	});///each
	console.log("sum : " +sum);
	$("#total_payment").attr("value",sum);
	sum = String(sum).replace(/(\d)(?=(?:\d{3})+(?!\d))/g, '$1,');
	$("#block_unit_won").text(sum);
}//sum
