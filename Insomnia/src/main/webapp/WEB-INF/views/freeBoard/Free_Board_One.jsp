...
<script>
	$(function(){
		$(".ilikeyou").click(function(){
			$.ajax({
				url:"<c:url value='/freeBoard/Free_Board_like.ins?no=${no}'/>",
				dataType:"JSON",
				success:function(data){
					$.each(data, function(key, value){
						if(value.yesorno == 'Y'){
							console.log(value.yesorno);
							alert("좋아요를 선택했습니다.");
							$(".like_up").text(value.like);
						}else{
							alert("이미 좋아요/싫어요를 선택했습니다.");
							return;
						}
					});
				},
		...
		$(".ihateyou").click(function(){
			$.ajax({
				url:"<c:url value='/freeBoard/Free_Board_hate.ins?no=${no}'/>",
				dataType:"JSON",
				success:function(data){
					$.each(data, function(key, value){
						if(value.yesorno == 'Y'){
						...
						<div class="d-flex justify-content-center row">
							<div class="col-md-12">
								<div class="section-title style-four" id="ad">
									<h2>V I E W&ensp;&ensp;&ensp;D E T A I L S</h2>
								</div>
							</div>
							<!-- /.col-xl- -->
						</div>
					...
					<div class="row">
						<div class="col-md-10" style="margin-left: -50px;">
							<form class="form-horizontal" onsubmit="return formCheck(this)"
								...
								method="post" action="<c:url value='/freeBoard/Free_Board_Modified.ins?no=${no }'/>"
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<c:if test="${id eq list.id }" var="iswriter">
											...
											<a href="<c:url value='/freeBoard/Free_Board_Delete.ins?no=${no }'/>"><input type="button" style="margin-left: 646px;margin-top: -30px" class="btn btn-danger btn_delete" value="삭제"></a>
										</c:if>
										<c:if test="${not iswriter }">
											<div style="margin-top: 6px; border: 1px solid; text-align: center;">${list.title}</div>
										</c:if>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-5">
										...
										<c:if test="${not iswriter }">
											<div style="width: 658px;height: 600px;margin-top: 6px; border: 1px solid;"> ${list.content }</div>
										</c:if>
									</div>
								</div>
								<div class="row">
									<div class="col-md-12 text-center" style="margin-left: 75px; margin-top: 15px">
										<c:if test="${id eq list.id }" var="iswriter">
											<input style="margin-right: 20px;" type="submit" class="btn btn-info" value="수정" />
											<a href="<c:url value='/freeBoard/Free_Board.ins'/>"><input type="button" class="btn btn-info" value="목록" /></a>
										</c:if>
										<c:if test="${not iswriter }">
											<a href="<c:url value='/freeBoard/Free_Board.ins'/>"><input type="button" class="btn btn-info" value="목록" style="margin-left: 597px; margin-top: -31px;"/></a>
										</c:if>
									...
									<div style="position: absolute; margin-left: 357px; margin-top: 100px;">
										...
										<a class ="ihateyou" style="cursor: pointer;"><img alt="" src="<c:url value='/img/downbtn.png'/>"></a>
									</div>
									<div style="position: absolute; margin-left: 409px; margin-top: 198px; display: inline-flex;">
										<h2 class="like_up" style="margin-right: 122px;">${list.ilikeyou }</h2>
										<h2 class="hate_up">${list.ihateyou }</h2>
									</div>
								</div>
							</form>
						</div>
